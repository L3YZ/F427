...
void  OS_CPU_SysTickInit (CPU_INT32U  cnts)
{
    ...
    prio                  |= DEF_BIT_MASK(OS_CPU_CFG_SYSTICK_PRIO, 24);  // 默认优先级为0，最高可设置的优先级

    CPU_REG_NVIC_SHPRI3    = prio;

                                                            /* Enable timer.                                          */
    CPU_REG_NVIC_ST_CTRL  |= CPU_REG_NVIC_ST_CTRL_CLKSOURCE |       // 4   
                             CPU_REG_NVIC_ST_CTRL_ENABLE;           // 使能  systick timer
                                                            /* Enable timer interrupt.                                */
    CPU_REG_NVIC_ST_CTRL  |= CPU_REG_NVIC_ST_CTRL_TICKINT;          // 使能中断
}
