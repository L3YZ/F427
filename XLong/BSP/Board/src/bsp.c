...
void  BSP_Init (void)
{
    ...
    BSP_REG32_RCC_CFGR  = (CPU_INT32U)0u;                       /* Reset CFGR register                                  */
	                                                            // HSI oscillator selected as system clock , 无分频

    BSP_REG32_RCC_CR    &= 0xFEF6FFFFu;                         /* Reset HSEON, CSSON and PLLON bits 关闭               */
	                                                            // PLL OFF  
    ...
    BSP_REG32_RCC_PLLCFGR = ( BSP_BIT_RCC_PLLCFGR_PLLM       )               |   
                            ...
                            ( BSP_MSK_PLLCFGR_PLLSRC_HSE     )               |  // 选择外部时钟源HSE作为PLL时钟输入
                            ((BSP_BIT_RCC_PLLCFGR_PLLP >> 1u ) -1u ) << 16u  |   
                            ( BSP_BIT_RCC_PLLCFGR_PLLQ << 24u);                 

    DEF_BIT_SET( BSP_REG32_RCC_CR, BSP_BIT_RCC_CR_PLLON);       // 启动 Main PLL  ，并等待锁相环稳定输出
	...
	DEF_BIT_SET(BSP_REG32_RCC_CFGR, BSP_MSK_SYSCLK_SRC_PLLCLK); // 将Main PLL 设置为SYSCLK
	...
	// 将所有TIM的内部时钟CK_IN频率都设置为HCLK=168M
	RCC_TIMCLKPresConfig(RCC_TIMPrescActivated); // 使能TIMPRE，因为APBx的预分频值为APB2-2和APB1-4，所以TIMxCLK=HCLK=168M
...
*
* Description : 分配XLong 引脚功能，并对普通IO功能引脚赋初值/设置上（下）拉
*
* 引脚分配整理
