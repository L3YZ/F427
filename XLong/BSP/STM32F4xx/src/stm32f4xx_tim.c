...
void TIM_OC1Init(TIM_TypeDef* TIMx, TIM_OCInitTypeDef* TIM_OCInitStruct)
{
  ...
  TIMx->CCER &= (uint16_t)~TIM_CCER_CC1E;           // OC1 is not active,禁止将OC1信号连接到对应功能引脚
  ...
  tmpccmrx &= (uint16_t)~TIM_CCMR1_OC1M;         //  禁用比较功能
  tmpccmrx &= (uint16_t)~TIM_CCMR1_CC1S;         //  设置为输出
  /* Select the Output Compare Mode */
  tmpccmrx |= TIM_OCInitStruct->TIM_OCMode;      //  设置具体输出模式
  
  /* Reset the Output Polarity level */
  tmpccer &= (uint16_t)~TIM_CCER_CC1P;
  /* Set the Output Compare Polarity */
  tmpccer |= TIM_OCInitStruct->TIM_OCPolarity;   //  设置输出极性，有效电平高/低
  
  /* Set the Output State */
  tmpccer |= TIM_OCInitStruct->TIM_OutputState;  //   OC1 signal is output on the corresponding output pin
